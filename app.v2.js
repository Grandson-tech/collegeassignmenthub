...
if (calcServiceType && calcAcademicLevel && calcPages && calcUrgency && calcBasePrice && calcLevelMultiplier && calcUrgencyFee && calcSubtotal && calcDiscount && calcTotalPrice) {
            const pricingConfig = {
            services: {
                'research-paper': { basePrice: 20 },
                'thesis': { basePrice: 25 },
                'literature-review': { basePrice: 12 },
                'case-study': { basePrice: 18 },
                'essay': { basePrice: 15 },
                'proctored-exam': { basePrice: 50 },
                'professional-exam': { basePrice: 60 },
                'class-management': { basePrice: 30 }
            },
        academicLevels: {
